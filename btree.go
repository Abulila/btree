...
// FromSnapshot runs in constant time.
...
// goroutines, but Read operations are, EXCEPT for Snapshot().
type BTree struct {
	copied  bool
	tree    *ImmutableBTree
	context btreeContext
...
// Set makes this instance share nodes with tree.
// This sharing of nodes allows Set to run in constant time.
// Subsequent changes to this instance after this call copy only the nodes
// that change. As this instance acquires its own copy of more and more nodes,
// each subsequent change copies fewer and fewer nodes.
// If len(tree) far exceeds the number of changes to be made,
// calling Set on this instance and making those changes is generally much
// faster than copying the items from tree to this instance and making those
// same changes.
...
// Snapshot returns a snapshot of this btree with the same degree.
// Snapshot is NOT safe to use with multiple goroutines as it changes
// this instance's state by marking its nodes as shared with the
// returned tree. This sharing of nodes allows Snapshot runs in constant time.
// Subsequent changes to this instance after this call copy only the nodes
// that change. As this instance acquires its own copy of more and more nodes,
// each subsequent change copies fewer and fewer nodes.
